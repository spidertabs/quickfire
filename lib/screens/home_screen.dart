...
class _HomeScreenState extends State<HomeScreen> {
          ...
          child: _notifications.isEmpty
            ...
            itemBuilder: (context, i) {
              ...
              return ListTile(
                ...
                tileColor:
                isRead ? null : AppTheme.primary.withValues(alpha: 0.05),
                title: Text(n['title'],
                    style: TextStyle(
                        color: AppTheme.getTextPrimary(context),
                        fontWeight:
  ...
  String _getViewTitle() {
    switch (_currentView) {
      case HomeView.dashboard:
        return 'Dashboard';
      case HomeView.assessments:
        return 'Tasks';
      case HomeView.results:
        return 'Preview';
    ...
    return Drawer(
      ...
      child: Column(
        children: [
          ...
          _drawerItem(
              HomeView.assessments, 'Tasks', Icons.assignment_outlined),
      ...
      child: Column(
        ...
        children: [
          ...
          Text('Here is your academic overview.',
              style: TextStyle(
                  color: AppTheme.getTextMuted(context), fontSize: 14)),
          const SizedBox(height: 32),

          // ── STAT CARDS ──────────────────────────────────────────────────
          // On narrow screens use a column of full-width cards so nothing truncates.
          isWide
              ? GridView.count(
            crossAxisCount: 3,
            crossAxisSpacing: 12,
            ...
            children: [
              _statCard('Enrolled Courses',
                  _enrolledCourseIds.length.toString(),
                  Icons.school_outlined, AppTheme.primary,
                  onTap: () =>
                      setState(() => _currentView = HomeView.courses)),
              _statCard('Active Assessments',
                  _assessments.length.toString(),
                  Icons.assignment_outlined, Colors.purple,
                  onTap: () => setState(
                          () => _currentView = HomeView.assessments)),
              _statCard(
                  'Pending Work',
                  _assessments
                      .where((asmt) => !_attempts.any((att) =>
                  att['assessment_id'] == asmt.id &&
            ...
            children: [
              ...
              _statCardWide(
                ...
                Colors.purple,
                onTap: () =>
                    setState(() => _currentView = HomeView.assessments),
              ),
              const SizedBox(height: 12),
              _statCardWide(
                'Pending Work',
  ...
  // Full-width stat card for narrow screens — label never truncates.
  ...
  Widget _buildRecentActivity() {
    final List<Map<String, dynamic>> activities = [
      ..._attempts.map((a) => {
        'type': 'assessment',
        'title':
        (a['quickfire_assessments']?['title'] ?? 'Assessment'),
        'subtitle':
        (a['quickfire_assessments']?['courses']?['code'] ?? ''),
        ...
        'remaining': a['status'] == 'submitted'
            ? null
            : _getRemainingTime(a,
            (a['quickfire_assessments']?['duration_minutes'] as int?)),
        'color': a['status'] == 'submitted'
            ? AppTheme.success
            : AppTheme.warning,
        'icon': a['status'] == 'submitted'
            ? Icons.check_rounded
            : Icons.timer_outlined,
        'onTap': () {
          final assessData =
              a['quickfire_assessments'] as Map<String, dynamic>? ?? {};
            ...
            Navigator.push(
                ...
                MaterialPageRoute(
                    builder: (_) => AssessmentScreen(
                        assessment: Assessment.fromJson(assessData),
                        student: student)));
          }
        },
      }),
          ...
          child: Column(
            children: [
              Icon(Icons.history_rounded,
                  color: AppTheme.getTextMuted(context), size: 40),
              const SizedBox(height: 12),
              Text('No recent activity',
                  style: TextStyle(
                      color: AppTheme.getTextSecondary(context),
                      fontWeight: FontWeight.w600)),
              Text('Start a task or enroll in a course to see it here.',
                  style: TextStyle(
  ...
  // Square grid card kept for wide screens only.
    ...
    if (_assessments.isEmpty) {
      if (_enrolledCourseIds.isEmpty) {
        return _emptyState(
          ...
          subtitle:
          'Go to the Courses section to subscribe to courses and see assessments.',
        );
      }
      return _emptyState(
        ...
        subtitle:
        'Your lecturers haven\'t posted any work yet for your enrolled courses.',
      );
    }
          ...
          children: [
            Row(
              children: [
                ...
                const Spacer(),
                if (isSubmitted)
                  _badge('Completed', AppTheme.success)
                else if (isInProgress)
                  _badge(
                      'Time: ${_getRemainingTime(attempt, assessment.durationMinutes)}',
                      AppTheme.warning)
                else
                  _badge('Available', AppTheme.getTextMuted(context)),
              ],
            ),
            const SizedBox(height: 12),
  ...
  Widget _buildResultsList() {
    final submitted =
    _attempts.where((a) => a['status'] == 'submitted').toList();
    ...
    if (submitted.isEmpty) {
      return _emptyState(
        ...
        subtitle:
        'Submit an assessment to preview your responses and academic record here.',
      );
    }
    ...
    return ListView.builder(
      ...
      itemBuilder: (context, i) {
        ...
        final assessData =
            attempt['quickfire_assessments'] as Map<String, dynamic>? ?? {};
        final courseData =
            assessData['courses'] as Map<String, dynamic>? ?? {};
            ...
            onTap: () => Navigator.push(
                ...
                MaterialPageRoute(
                    builder: (_) => ReportScreen(
                        ...
                        totalScore: attempt['total_score'] ?? 0,
                        showResults: false))),
            title: Text(assessData['title'] ?? 'Assessment',
                style: TextStyle(
                    color: AppTheme.getTextPrimary(context),
                    fontWeight: FontWeight.bold)),
            subtitle: Text(
  ...
  Widget _buildCoursesList() {
    ...
    final enrolled =
    filtered.where((c) => _enrolledCourseIds.contains(c['id'])).toList();
    final available =
    filtered.where((c) => !_enrolledCourseIds.contains(c['id'])).toList();
        ...
        children: [
          ...
          if (isEnrolled)
            ...
            ElevatedButton(
              onPressed: () async {
                setState(() => _loading = true);
                final ok = await SupabaseService.enrollInCourse(
  ...
  void _showUnenrollDialog(Map<String, dynamic> course) {
    showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        content: const Text(
            'You will no longer be able to see assessments for this course.'),
        actions: [
          TextButton(
              onPressed: () => Navigator.pop(context),
          ...
          TextButton(
            onPressed: () async {
              Navigator.pop(context);
              setState(() => _loading = true);
              final ok = await SupabaseService.unenrollFromCourse(
                  student.id, course['id']);
              if (ok) {
                _loadData();
              } else {
    ...
    return Center(
      child: Column(
        ...
        children: [
          Icon(icon,
              size: 80,
              color: AppTheme.getTextMuted(context).withValues(alpha: 0.3)),
          const SizedBox(height: 16),
          Text(title,
              style: TextStyle(
                  color: AppTheme.getTextPrimary(context),
